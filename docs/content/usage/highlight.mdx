...
Tree-sitter rescans its in-progress match list before it emits each capture, so a query pattern that stays open across a large subtree costs more the deeper the document nests. Lumis bounds the number of matches kept in progress at 4096, which keeps highlighting linear on documents like a large HTML table wrapped in a single element.

Raise it when a document is large enough that tree-sitter drops matches and scopes go missing, and accept the extra time. The maximum is 65536.
